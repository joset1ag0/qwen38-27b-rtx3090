...
- A codex profile in `~/.codex/kat.config.toml` (codex keeps each profile in
  ...
  # silences the "Model metadata for `kat-coder-v2.5` not found" warning —
  # generated by scripts/codex-kat-catalog.py; regenerate after codex updates
  model_catalog_json = "/home/tiago/.codex/kat-models.json"
  ```

  The catalog file replaces codex's entire bundled model catalog for the
  process, so [`scripts/codex-kat-catalog.py`](scripts/codex-kat-catalog.py)
  downloads the bundled `models.json` and `prompt.md` from the installed codex
  version's tag and appends a `kat-coder-v2.5` entry shaped like codex's
  internal fallback (same base instructions), but with the server's real
  `KAT_CTX` context window instead of the fallback's 272k.
